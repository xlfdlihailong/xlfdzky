...
int xTableWidget::getIndexByNameCol(QString strNameCol)
{
    int iIndexFind=-1;
...
QString xTableWidget::getItemTextNowByColIndex(int col)
{
    int rowNow=this->getRowNow();
    if(rowNow<0)
    {
        qlib::messageBox("当前位置没有有效的行");
        return "";
    }
    return this->getItemText(rowNow,col);
}

QString xTableWidget::getItemTextNowByColName(QString strNameCol)
{
    int index=this->getIndexByNameCol(strNameCol);
    if(index<0)
    {
        qlib::messageBox(QString("找不到列名为")+strNameCol+"的列");
        return "";
    }
    return this->getItemTextNowByColIndex(index);
}
...
QAction *xTableWidget::newAction(QString name)
{
...
void xTableWidget::setColWidth(QString strNameCol, int width)
{
    int index=this->getIndexByNameCol(strNameCol);
    if(index<0)
    {
